...
這個專案適合個人學習與本機練習，不是已經可直接公開上線的多人線上 judge。
...
- 使用 Monaco 編輯器撰寫 Java 或 Python3 解答。
- 執行可見測資、執行一筆自訂 JSON 輸入，或提交並跑可見加隱藏測資。
- 自動儲存每題進度，包含已選語言與各語言的程式碼。
...
- 顯示 AC、WA、CE、RE、TLE 結果、各測資細節、執行時間，以及編譯錯誤在編輯器中的標記。
...
- 如果要用本機沙盒執行 Python3，需要 Python 3。
...
4. 編輯起始程式碼。
5. 按 `Run` 執行可見測資或目前自訂輸入。
6. 按 `Submit` 執行可見與隱藏測資。

只有 `Submit` 可以把題目標成已通過，因為 `Run` 不會執行隱藏測資。
...
`problem.json` 定義標題、難度、標籤、題目敘述、範例、限制條件、支援語言、函式名稱、參數、回傳型別與顯示用函式簽名。
